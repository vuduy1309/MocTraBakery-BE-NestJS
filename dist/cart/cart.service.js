...
let CartService = class CartService {
    ...
    async getCartByUser(userId) {
        const cart = await this.cartModel.findOne({ userId: new mongoose_2.Types.ObjectId(userId) }).populate('items.productId').exec();
        if (!cart)
    ...
    async addToCart(userId, productId, size, quantity) {
        ...
        if (!cart) {
            return this.cartModel.create({
                ...
                items: [{ productId: new mongoose_2.Types.ObjectId(productId), size, quantity, price }],
            });
        }
        ...
        if (idx > -1) {
            ...
            cart.items.push({ productId: new mongoose_2.Types.ObjectId(productId), size, price, quantity });
        }
        return cart.save();
    }
